...
@section('content')
{{-- Stuktur HTML tidak diubah, hanya CSS yang disesuaikan --}}
...
<style>
    /* Styling untuk container baru di desktop */
    .create-post-container {
        padding: 0.2rem 0; /* Memberi jarak atas bawah di desktop */
    }
    ...
    /* ... (semua style lain yang tidak berubah) ... */
    ...
    .image-upload-area { display: block; border: 2px dashed var(--border-color); border-radius: 16px; background: linear-gradient(135deg, var(--hover-color) 0%, rgba(var(--primary-color-rgb, 0, 123, 255), 0.02) 100%); padding: 3rem 2rem; text-align: center; cursor: pointer; transition: all 0.3s ease; overflow: hidden; }
    .image-upload-area:hover, .image-upload-area.highlight { border-color: var(--primary-color); background: linear-gradient(135deg, rgba(var(--primary-color-rgb, 0, 123, 255), 0.05) 0%, rgba(var(--primary-color-rgb, 0, 123, 255), 0.02) 100%); transform: translateY(-2px); box-shadow: var(--shadow-medium); }
    .upload-icon i { font-size: 3rem; color: var(--primary-color); opacity: 0.8; transition: all 0.3s ease; }
    .image-upload-area:hover .upload-icon i { transform: translateY(-5px); opacity: 1; }
    .upload-title { font-size: 1.1rem; font-weight: 600; color: var(--text-primary); margin-bottom: 0.5rem; }
    .upload-subtitle { font-size: 0.85rem; color: var(--text-secondary); margin: 0; }
    .image-preview-container { background: var(--card-background); border: 2px solid var(--border-color); border-radius: 16px; padding: 1.5rem; margin-top: 1rem; animation: fadeIn 0.3s ease; }
    @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
    .preview-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid var(--border-color); }
    .preview-title { font-weight: 600; color: var(--text-primary); font-size: 0.9rem; }
    .image-preview { text-align: center; margin-bottom: 1rem; }
    .image-preview img { max-width: 100%; max-height: 300px; border-radius: 12px; box-shadow: var(--shadow-light); object-fit: cover; }
    .image-info { display: flex; justify-content: space-between; align-items: center; font-size: 0.8rem; color: var(--text-secondary); }
    .file-name { font-weight: 500; max-width: 60%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .file-size { font-weight: 600; color: var(--primary-color); }
    ...
    /* ===================================
        PERBAIKAN UNTUK TABLET DAN MOBILE
       =================================== */

    /* Aturan untuk TABLET (misal: iPad potret) */
    @media (max-width: 991.98px) {
        .create-post-container {
            padding-top: 0.7rem; /* Mengurangi padding atas untuk tablet */
        }
        .container-inner {
            padding-left: 1.5rem;  /* Menambah padding samping untuk tablet */
            padding-right: 1.5rem;
        }
    }

    /* Aturan untuk MOBILE (misal: iPhone) */
    @media (max-width: 767.98px) {
        ...
        .create-post-container {
            padding-top: 5px; /* Hapus padding luar di mobile */
            padding-bottom: 5px;
        }
        .container-inner {
            padding: 0; /* Hapus padding dalam di mobile */
        }
        .create-post-card {
            /* PENTING: Menggunakan !important untuk mengalahkan class .rounded-4 */
            border-radius: 0 !important; 
            box-shadow: none !important;
            padding: 1.5rem !important; /* Memberi padding internal yang konsisten */
        }
    }
</style>
